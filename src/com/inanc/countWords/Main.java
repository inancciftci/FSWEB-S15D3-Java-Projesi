...
public class Main {
    public static void main(String[] args) {
        ...
        dummyData = dummyData.toLowerCase();

        String[] toArr = dummyData.split(" ");

        Map<String, Integer> dummyToMap = new TreeMap<>();
        for(String word : toArr) {
            if(!dummyToMap.containsKey(word)) {
                dummyToMap.put(word, 1);
            } else {
                dummyToMap.put(word, dummyToMap.get(word) + 1);
            }
        }
        System.out.println(dummyToMap);
    }
}
